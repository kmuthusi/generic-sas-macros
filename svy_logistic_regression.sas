...
/*
%* macro to validate input parameters;
%macro svy_validate();
...
%if %sysfunc(exist(&dataset)) eq 0 %then %do;
	%put ERROR: Specified dataset &dataset. does not exist;
	%put ERROR: The macro ended abnormally.;
	%abort cancel;
%end;
...
%let nobs=%sysfunc(close(&dsid.,nobs));
%let rc=%sysfunc(close(&dsid.));
%if &nobs. eq 0 %then %do; 
	%put ERROR: Specified dataset &dataset. is empty;
	%put ERROR: The macro ended abnormally.;
	%abort cancel;
%end; 

%* check if outcome is specifies;
%if %length(&outcome) = 0 %then %do;
	%put ERROR: outcome= must be specified;
	%put ERROR: The macro ended abnormally.;
	%abort cancel;
%end;

%* check if outevent is specified;
%if %length(&outevent) = 0 %then %do;
	%put ERROR: outevent= must be specified;
	%put ERROR: The macro ended abnormally.;
	%abort cancel;
%end;

%* check if outevent is a valid value label for &outcome;
ods output OneWayFreqs=&outcome;
proc freq data=&dataset;
	table &outcome;
run;

proc sql;
	create table t_&outcome as select * from &outcome where trim(left(f_&outcome) eq trim(left(&outevent));
quit;

%let dsid=%sysfunc(open(t_&outcome));
%let nobs=%sysfunc(close(&dsid.,nobs));
%let rc=%sysfunc(close(&dsid.));
%if &nobs. eq 0 %then %do; 
	%put ERROR: &outevent. is not a valid value label for &outcome.;
	%put ERROR: The macro ended abnormally.;
	%abort cancel;
%end; 

%* check if either a categorical or continous variable is specified;
%if %length(&catvar)=0 and %length(&contvar)=0 %then %do;
	%put ERROR: Either catvar= or contvar= must be specified;
	%put ERROR: The macro ended abnormally.;
	%abort cancel;
%end;

%* if a categorical variable is specified, check if class variable is also specified;
%if %length(&catvar)!=0 and %length(&class)=0 %then %do;
	%put ERROR: CLASS Statement for &catvar must be specified;
	%put ERROR: The macro ended abnormally.;
	%abort cancel;
%end;

%* if a categorical variable is specified, check if value of class statement valid;
ods output OneWayFreqs=&catvar;
proc freq data=&dataset;
	table &catvar;
run;
...
data &catvar;
set &catvar;
	classvar= trim(left(&catvar))|| trim(left(ref=%bquote(f_&catvar)));
run;

proc sql;
	create table t_&catvar as select * from &catvar where trim(left(f_&catvar) eq trim(left(classvar));
quit;

%let dsid=%sysfunc(open(t_&catvar));
%let nobs=%sysfunc(close(&dsid.,nobs));
%let rc=%sysfunc(close(&dsid.));
%if &nobs. eq 0 %then %do; 
	%put ERROR: class variable is not a valid value label for &catvar.;
	%put ERROR: The macro ended abnormally.;
	%abort cancel;
%end; 

%* check if any other error exist;
%if &syserr. ne 0 %then %do;
%abort cancel;
%end;
